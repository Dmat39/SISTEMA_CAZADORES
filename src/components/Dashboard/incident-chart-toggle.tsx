...
export function IncidentChartToggle() {
  ...
  return (
    <div className="rounded-2xl bg-[#fdfbf5] dark:bg-[#111827] shadow border border-[#e8dfc8] dark:border-white/10 overflow-hidden h-full flex flex-col">
        ...
        {metrics && isLineActive && (
          <div className="mt-2 grid grid-cols-2 gap-2">
            {([
              ...
              <div key={label} className="rounded-xl bg-[#f0e6d0] dark:bg-white/5 px-2 py-2">
                <p className={`text-[10px] font-bold uppercase tracking-wide text-center ${accent} mb-2`}>{label}</p>
                <div className="flex items-stretch divide-x divide-[#e8dfc8] dark:divide-white/10">
                  {([
                    ...
                    <div key={l} className="flex-1 text-center px-1.5">
                      <p className="text-base font-bold text-[#3d2f1f] dark:text-white leading-none">{v}</p>
                      <p className="text-[10px] text-[#7a6a52] dark:text-gray-400 mt-0.5">{l}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="px-2 py-4 flex-1 flex flex-col justify-center">
        {loading ? (
          <div className="h-[345px] flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-[#a89878]">Cargando datos...</p>
          </div>
      ...
      {/* Legend toggles */}
      {processedData.length > 0 && (
        <div className="px-4 pb-2 flex items-center justify-center gap-3 border-t border-[#e8dfc8] dark:border-white/8 pt-2">
          {([
            ...
            <button
              ...
              className={[
                ...
                active
                  ? 'bg-[#f0e6d0] dark:bg-white/8 text-[#7a6a52] dark:text-gray-200'
                  : 'opacity-40 text-[#7a6a52] dark:text-gray-400',
              ].join(' ')}
            >
              <span className={`w-3 h-3 rounded-sm ${active ? color : 'bg-[#e8dfc8] dark:bg-gray-600'}`} />
              {label}
            </button>
          ))}
        </div>
