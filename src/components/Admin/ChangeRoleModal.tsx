            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
                <Dialog.Panel className={`rounded-lg shadow-lg max-w-md w-full p-6 ${isDark ? 'bg-gray-800' : 'bg-[#fdfbf5]'}`}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="mb-6">
                            <label className={`block text-sm font-medium mb-1 ${isDark ? 'text-gray-200' : 'text-[#3d2f1f]'}`}>Nuevo rol</label>
                            <select
                                value={role}
                                onChange={(e) => setRole(e.target.value as UserRole)}
                                className={`w-full border px-3 py-2 rounded ${isDark ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-[#fdfbf5] border-[#e8dfc8] text-[#3d2f1f]'}`}
                            >
