...
const IncidenciaCard = ({
  ...
  return (
    <Card
      className="relative overflow-hidden border-[#e8dfc8] dark:border-white/10 bg-[#fdfbf5] dark:bg-[#111827] shadow-sm hover:shadow-lg hover:shadow-black/5 dark:hover:shadow-black/40 transition-all duration-200 hover:-translate-y-0.5 select-none"
      style={{ cursor: "grab" }}
      draggable={true}
      ...
      <CardContent className="pl-5 pr-4 py-3 flex flex-col h-full">
        <div className="flex flex-col gap-3 flex-1">
          ...
          <div className="flex items-start justify-between gap-2 min-h-[2.5rem]">
            <h3 className="font-semibold text-sm text-[#3d2f1f] dark:text-white text-pretty leading-tight line-clamp-2">
              {name || "Sin título"}
            </h3>
          ...
          {/* Description */}
          <p className="text-xs text-[#7a6a52] dark:text-gray-300 text-pretty line-clamp-3 min-h-[3rem]">
            {description || "Sin descripción"}
          </p>

          {/* Metadata */}
          <div className="space-y-2 mt-auto pt-1">
            <div className="flex items-center gap-4 text-xs text-[#a89878] dark:text-gray-400">
