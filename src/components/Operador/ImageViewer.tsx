    ...
    <>
      <div className="absolute top-2 right-3 flex gap-2">
        <button
          ...
          title="Ver archivo"
          className="text-[#a89878] hover:text-[#3d2f1f] transition-all duration-200 ease-in cursor-pointer"
        >
      ...
      {loading && <p className="text-sm text-[#a89878]">Cargando...</p>}
      {error && <p className="text-sm text-red-500">{error}</p>}

      {isVisible && mediaUrl && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[9999]">
          <div className="relative">
            <div className="bg-[#fdfbf5] px-6 py-8 rounded-lg max-w-4xl max-h-[90vh] overflow-auto">
              <div className="absolute bg-[#fdfbf5] w-10 h-10 -top-4 -right-4 rounded-full flex items-center justify-center shadow-lg">
                <button
                  onClick={toggleVisibility}
                  className="absolute top-0.5 right-0.5 text-[#a89878] hover:text-[#7a6a52] cursor-pointer"
                >
